...
## Examples
### ESP32
The Sourceful Energy Zap is an ESP32 project (P1 smart energy meter reader) under development at: https://github.com/srcfl/srcful-zap-firmware
...
- Important REST endpoints for onboarding
...
If software keys are used it is imperative that the private key is stored on the device itself. The private key must not be stored in plain text in files, code, nor as environment variables. If a private key is regarded as compromised by the network its corresponding public key will be banned in the SEN, potentially resulting in loss of rewards and other negative effects.
