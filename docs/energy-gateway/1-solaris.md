...
The Srcful Energy Gateway (batch #1: "Solaris") is a hardware device designed to connect Distributed Energy Resources (DERs) to the Srcful network of gateways. It is based on the Rasberry Pi 4 platform and is compatible with a growing number of inverters. 
...
- Support for major inverters: The gateway is compatible with major inverters, making it easy to integrate with existing DER installations.
...
- Helium support: The gateway has Helium support, enabling it to connect to the Helium network for added connectivity options. (Not on pre-order models)
...
- Proof-of-Source and Proof-of-Control: The gateway can perform Proof-of-Source and Proof-of-Control to ensure that energy transactions on the Srcful network are secure and verified. Proof-of-Control is planned to be launced 2024.
...
2. Configure the gateway: Once the gateway is connected, you can configure it using the web-based interface found at [config.srcful.io](https://config.srcful.io). This allows you to set up the gateway for your specific DER and network requirements. (An Web-BT-compatible device is needed. Typically a Android phone or laptop. IPhone do not support Web-BT as of Q2 2023)
  - Wifi
  - Your Solana public wallet address
  - GPS-coordinates of you GW.

3. Start using the gateway: Once the gateway is configured, it is ready to use. You can start monitoring your DER's performance and participating in peer-to-peer energy trading.
